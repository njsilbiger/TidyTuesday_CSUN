...
## filter out all drinks that have bitters in them
...
sites.ingred<- data.frame(ingreds.mds$points) #save species intrinsic values into dataframe
...
nmds.plot <- ggplot(# run an nMDS 
  ...
  xlab("")+
  ylab("")+
#   ggtitle("Drinks with bitters as an ingredient") +
  scale_color_fish_d(option = "Naso_lituratus")+
...
species<-nmds.plot + 
  ...
  geom_point(aes(MDS1, MDS2, color = glass), size = 3)+ #adds site points to plot, shape determined by Landuse, colour determined by Management
  labs(colour = "Glass Type") # add legend labels for Management and Landuse
...
# put the plots together in patchword
