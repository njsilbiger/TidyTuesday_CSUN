## Look at the big cars data and make a ridgeplot
...
# Plot
  ggplot(big_epa_cars, aes(x = co2, y = reorder(VClass, -pv2), fill = ..x..), group = Auto_manual) + #ordered axis by decreasing car size
    geom_density_ridges_gradient(scale = 3, rel_min_height = 0.01) + # make the ridges
    scale_fill_viridis(name = "Vehicle size class", option = "C") + # change the color
